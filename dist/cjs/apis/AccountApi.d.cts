...
export declare class AccountApi extends runtime.BaseAPI {
     ...
     * 코드 교환하기
     */
    redeemCodeRaw(requestParameters: RedeemCodeOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<void>>;
    /**
     * 리딤 코드와 배지, 배경 등을 교환합니다.  **주의**: 로그인이 필요한 API는 사이트 내에서 호출할 수 없으므로 별도 도구를 이용해주십시오.
     * 코드 교환하기
     */
    redeemCode(requestParameters: RedeemCodeOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<void>;
    /**
     * 계정의 설정을 변경합니다.  **주의**: 로그인이 필요한 API는 사이트 내에서 호출할 수 없으므로 별도 도구를 이용해주십시오.
     * 계정 설정 변경하기
     */
    updateAccountSettingsRaw(requestParameters: UpdateAccountSettingsOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<void>>;
    /**
     * 계정의 설정을 변경합니다.  **주의**: 로그인이 필요한 API는 사이트 내에서 호출할 수 없으므로 별도 도구를 이용해주십시오.
     * 계정 설정 변경하기
     */
    updateAccountSettings(requestParameters: UpdateAccountSettingsOperationRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<void>;
    /**
     * 현재 로그인한 계정 정보를 가져옵니다.  **주의**: 로그인이 필요한 API는 사이트 내에서 호출할 수 없으므로 별도 도구를 이용해주십시오.
     * 로그인 정보 가져오기
     */
    verifyAccountCredentialsRaw(requestParameters: VerifyAccountCredentialsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<runtime.ApiResponse<VerifyCredentialsCredential>>;
    /**
     * 현재 로그인한 계정 정보를 가져옵니다.  **주의**: 로그인이 필요한 API는 사이트 내에서 호출할 수 없으므로 별도 도구를 이용해주십시오.
     * 로그인 정보 가져오기
     */
    verifyAccountCredentials(requestParameters?: VerifyAccountCredentialsRequest, initOverrides?: RequestInit | runtime.InitOverrideFunction): Promise<VerifyCredentialsCredential>;
}
